...
def targeted_flips(samples, args, trainer, generator, embedding_weight, bpe):
    ...
    for i in range(samples['ntokens'] * 3): # this many iters over a single example. Gradient attack will early stop
        ...
        new_found_input_tokens = None
        batch_size = 64
        all_inference_samples, _ = all_attack_utils.build_inference_samples_difficult(samples, batch_size, args, candidate_input_tokens, trainer, bpe, untouchable_token_blacklist=untouchable_token_blacklist, adversarial_token_blacklist=adversarial_token_blacklist)
